...
> когда надумаем. Связано с `toolbox/mkv-default-tracks/EDL-заметки.md` (раздел «Идея собственного
> Kodi-аддона»).

## Зачем

Утилита `toolbox/mkv-default-tracks` локально генерирует `.edl` рядом с видео
